...
import { type AddUserSchema, addUserSchema } from '@viastud/utils'
import { PlusIcon } from 'lucide-react'
import { useState } from 'react'
import { useForm } from 'react-hook-form'

export const AddUserModal = ({ refresh }: BaseFormModalProps) => {
  const addUserForm = useForm<AddUserSchema>({
    resolver: async (data, context, options) => {
      return zodResolver(addUserSchema)(data, context, options)
    },
    defaultValues: {
      lastName: '',
      firstName: '',
  ...
  const { mutateAsync: addUserMutation } = trpc.user.create.useMutation({
    onSuccess: () => {
      setOpen(false)
      refresh()
      addUserForm.reset()
  ...
  const onSubmit = async (data: AddUserSchema) => {
